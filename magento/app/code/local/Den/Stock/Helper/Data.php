 ...
 */
class Den_Stock_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'stock/stock/enabled';
    const XML_PATH_TITLE = 'stock/stock/title';
    const XML_PATH_MENU_LEFT = 'stock/stock/menuLeft';
    const XML_PATH_MENU_RIGHT = 'stock/stock/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'stock/stock/footerEnabled';
    const XML_PATH_LAYOUT = 'stock/stock/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $stockRouter = Mage::getStoreConfig('stock/stock/route', $store->getId());
                    if ($stockRouter == $router) {
                        $droute = Mage::getStoreConfig('stock/stock/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('stock/stock/enabled') && $this->extensionEnabled('Den_Stock');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('stock/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

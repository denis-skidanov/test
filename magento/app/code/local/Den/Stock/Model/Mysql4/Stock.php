...
class Den_Stock_Model_Mysql4_Stock extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('stock/stock', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on stock delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete stock/store
		$adapter->delete($this->getTable('stock/store'), 'post_id='.$object->getId());
		// 2. Delete stock/post_cat
		$adapter->delete($this->getTable('stock/post_cat'), 'post_id='.$object->getId());
		// 3. Delete stock/post_comment 
		$adapter->delete($this->getTable('stock/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('stock/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

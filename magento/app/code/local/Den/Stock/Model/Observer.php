 ...
 */
class Den_Stock_Model_Observer {
    ...
    public function addBlogSection($observer) {
        ...
        if (!($sitemapObject instanceof Mage_Sitemap_Model_Sitemap))
            throw new Exception(Mage::helper('stock')->__('Error during generation sitemap'));

        $storeId = $sitemapObject->getStoreId();
        $date = Mage::getSingleton('core/date')->gmtDate('Y-m-d');
        $baseUrl = Mage::app()->getStore($storeId)->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK);
        /**
         * Generate stock pages sitemap
         */
        $changefreq = (string) Mage::getStoreConfig('sitemap/stock/changefreq');
        $priority = (string) Mage::getStoreConfig('sitemap/stock/priority');
        $collection = Mage::getModel('stock/stock')->getCollection()->addStoreFilter($storeId);

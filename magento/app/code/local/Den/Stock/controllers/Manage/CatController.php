...
class Den_Stock_Manage_CatController extends Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/stock/cat');
    }

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('stock/cat')
        ...
        if(!is_array($stockIds)) {
            ...
            try {
                foreach ($stockIds as $stockId) {
                    $stock = Mage::getModel('stock/cat')->load($stockId);
                    $stock->delete();
                }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('stock/cat')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('stock/manage_cat_edit'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('stock')->__('Category does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('stock')->__('Unable to find category to save'));
        $this->_redirect('*/*/');
	}

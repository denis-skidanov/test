...
class Den_Stock_Block_Rss extends Mage_Rss_Block_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		if (Mage::getStoreConfig('stock/rss/image') != "")
		{
			$data['image'] = $this->getSkinUrl(Mage::getStoreConfig('stock/rss/image'));
		}
		...
		if ($cat_id = Mage::getSingleton('stock/cat')->load($identifier)->getcatId()){
			Mage::getSingleton('stock/status')->addCatFilterToCollection($collection, $cat_id);
		}
		
		
		Mage::getSingleton('stock/status')->addEnabledFilterToCollection($collection);
		
		$collection->setPageSize((int)Mage::getStoreConfig('stock/rss/posts'));

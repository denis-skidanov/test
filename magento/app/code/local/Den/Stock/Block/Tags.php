...
class Den_Stock_Block_Tags extends Mage_Core_Block_Template {


    public function _construct(){
        parent::_construct();
        return $this->setTemplate('aw_stock/tags.phtml');
    }
    ...
    public function getCollection(){
        ...
        $coll = Mage::getModel('stock/tag')->getCollection(); 
        $coll->getSelect()
                ->where('tag_count > 0')
                ->columns(array('tag_final_count'=>'SUM(tag_count)'))
                ->joinLeft(array("stores"=>$coll->getTable('stock/store')),'main_table.store_id = stores.store_id',array())
                ->joinLeft(array("stocks"=>$coll->getTable('stock/stock')),"stores.post_id = stocks.post_id",array())
                ->where('stocks.status = 1')
                ->where('FIND_IN_SET(main_table.tag, stocks.tags)')
                ->where('main_table.store_id = ? OR main_table.store_id = 0', Mage::app()->getStore()->getId())
                ->order(array('tag_final_count DESC',
                           'tag'))
                           
                ->limit(Mage::getStoreConfig(Den_Stock_Helper_Config::XML_TAGCLOUD_SIZE))
                ->group('tag');

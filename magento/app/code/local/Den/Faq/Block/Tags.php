...
class Den_Faq_Block_Tags extends Mage_Core_Block_Template {


    public function _construct(){
        parent::_construct();
        return $this->setTemplate('aw_faq/tags.phtml');
    }
    ...
    public function getCollection(){
        ...
        $coll = Mage::getModel('faq/tag')->getCollection(); 
        $coll->getSelect()
                ->where('tag_count > 0')
                ->columns(array('tag_final_count'=>'SUM(tag_count)'))
                ->joinLeft(array("stores"=>$coll->getTable('faq/store')),'main_table.store_id = stores.store_id',array())
                ->joinLeft(array("faqs"=>$coll->getTable('faq/faq')),"stores.post_id = faqs.post_id",array())
                ->where('faqs.status = 1')
                ->where('FIND_IN_SET(main_table.tag, faqs.tags)')
                ->where('main_table.store_id = ? OR main_table.store_id = 0', Mage::app()->getStore()->getId())
                ->order(array('tag_final_count DESC',
                           'tag'))
                           
                ->limit(Mage::getStoreConfig(Den_Faq_Helper_Config::XML_TAGCLOUD_SIZE))
                ->group('tag');

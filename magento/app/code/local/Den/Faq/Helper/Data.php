 ...
 */
class Den_Faq_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'faq/faq/enabled';
    const XML_PATH_TITLE = 'faq/faq/title';
    const XML_PATH_MENU_LEFT = 'faq/faq/menuLeft';
    const XML_PATH_MENU_RIGHT = 'faq/faq/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'faq/faq/footerEnabled';
    const XML_PATH_LAYOUT = 'faq/faq/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $faqRouter = Mage::getStoreConfig('faq/faq/route', $store->getId());
                    if ($faqRouter == $router) {
                        $droute = Mage::getStoreConfig('faq/faq/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('faq/faq/enabled') && $this->extensionEnabled('Den_Faq');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('faq/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

...
class Den_All_Model_Feed_Extensions extends Den_All_Model_Feed_Abstract
{
    ...
    {
        return Den_All_Helper_Config::EXTENSIONS_FEED_URL;
    }
    ...
    public function checkExtensions()
    {
        ...
        $magentoPlatform = Den_All_Helper_Versions::getPlatform();
        foreach ($modules as $extensionName) {
            if (strstr($extensionName, 'Den_') === false) {
                continue;
            }
            if ($extensionName == 'Den_Core' || $extensionName == 'Den_All') {
                continue;
            }
            if ($platformNode = $this->getExtensionPlatform($extensionName)) {
                $extensionPlatform = Den_All_Helper_Versions::convertPlatform($platformNode);

...
class Den_Job_Block_Tags extends Mage_Core_Block_Template {


    public function _construct(){
        parent::_construct();
        return $this->setTemplate('aw_job/tags.phtml');
    }
    ...
    public function getCollection(){
        ...
        $coll->getSelect()
                ...
                ->joinLeft(array("stores"=>$coll->getTable('job/store')),'main_table.store_id = stores.store_id',array())
                ->joinLeft(array("jobs"=>$coll->getTable('job/job')),"stores.post_id = jobs.post_id",array())
                ->where('jobs.status = 1')
                ->where('FIND_IN_SET(main_table.tag, jobs.tags)')
                ->where('main_table.store_id = ? OR main_table.store_id = 0', Mage::app()->getStore()->getId())
                ->order(array('tag_final_count DESC',
                           'tag'))
                           
                ->limit(Mage::getStoreConfig(Den_Job_Helper_Config::XML_TAGCLOUD_SIZE))
                ->group('tag');

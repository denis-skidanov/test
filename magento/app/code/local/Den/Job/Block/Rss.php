...
class Den_Job_Block_Rss extends Mage_Rss_Block_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		if (Mage::getStoreConfig('job/rss/image') != "")
		{
			$data['image'] = $this->getSkinUrl(Mage::getStoreConfig('job/rss/image'));
		}
		...
		if ($cat_id = Mage::getSingleton('job/cat')->load($identifier)->getcatId()){
			Mage::getSingleton('job/status')->addCatFilterToCollection($collection, $cat_id);
		}
		
		
		Mage::getSingleton('job/status')->addEnabledFilterToCollection($collection);
		
		$collection->setPageSize((int)Mage::getStoreConfig('job/rss/posts'));

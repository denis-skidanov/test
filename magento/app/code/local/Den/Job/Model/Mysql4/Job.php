...
class Den_Job_Model_Mysql4_Job extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('job/job', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on job delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete job/store
		$adapter->delete($this->getTable('job/store'), 'post_id='.$object->getId());
		// 2. Delete job/post_cat
		$adapter->delete($this->getTable('job/post_cat'), 'post_id='.$object->getId());
		// 3. Delete job/post_comment 
		$adapter->delete($this->getTable('job/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('job/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

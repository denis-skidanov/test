...
class Den_Job_Model_Observer {
    ...
    public function addBlogSection($observer) {
        ...
        if (!($sitemapObject instanceof Mage_Sitemap_Model_Sitemap))
            throw new Exception(Mage::helper('job')->__('Error during generation sitemap'));

        $storeId = $sitemapObject->getStoreId();
        $date = Mage::getSingleton('core/date')->gmtDate('Y-m-d');
        $baseUrl = Mage::app()->getStore($storeId)->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK);
        /**
         * Generate job pages sitemap
         */
        $changefreq = (string) Mage::getStoreConfig('sitemap/job/changefreq');
        $priority = (string) Mage::getStoreConfig('sitemap/job/priority');
        $collection = Mage::getModel('job/job')->getCollection()->addStoreFilter($storeId);

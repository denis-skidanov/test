 ...
 */
class Den_Job_Manage_JobController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('admin/job/posts');
    }

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('job/posts');
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('job/job')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->loadLayout();
            $this->_setActiveMenu('job/posts');
            $this->displayTitle('Edit post');

            $this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

            $this->_addContent($this->getLayout()->createBlock('job/manage_job_edit'))
                    ->_addLeft($this->getLayout()->createBlock('job/manage_job_edit_tabs'));

            $this->getLayout()->getBlock('head')->setCanLoadTinyMce(true);

            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('job')->__('Post does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function duplicateAction() {
        ...
        while (Mage::getModel('job/post')->loadByIdentifier($newIdentifier)->getData())
            $newIdentifier = $oldIdentifier . ++$i;
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($data['tags'])) {
                ...
                foreach ($tags as $key => $tag) {
                    $tags[$key] = Mage::helper('job')->convertSlashes($tag, 'forward');
                }
            ...
            try {
                ...
                foreach ($affectedTags as $tag) {
                    foreach ($stores as $store) {
                        if (trim($tag)) {
                            Mage::getModel('job/tag')->loadByName($tag, $store)->refreshCount();
                        }
                    }
                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('job')->__('Post was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('job')->__('Unable to find post to save'));
        $this->_redirect('*/*/');
    }
            ...
            try {
                ...
                foreach ($_tags as $tag)
                    foreach ($_stores as $store)
                        if (trim($tag)) {
                            Mage::getModel('job/tag')->loadByName($tag, $store->getId())->refreshCount();
                        }
        ...
        if (!is_array($jobIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                        $this->__('Total of %d record(s) were successfully updated', count($jobIds))
                );
            } catch (Exception $e) {
    ...
    protected function displayTitle($data = null, $root = 'Job') {
        ...
        if (!Mage::helper('job')->magentoLess14()) {
            if ($data) {
                ...
                $this->_title($this->__('Job'))->_title($root);
            }
        }
        return $this;
    }

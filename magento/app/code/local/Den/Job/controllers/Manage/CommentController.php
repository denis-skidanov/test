...
class Den_Job_Manage_CommentController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/job/comment');
    }
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('job/comment')
    ...
    public function massApproveAction()
    {
        ...
        if(!is_array($jobIds)) {
            ...
            try {
                foreach ($jobIds as $jobId) {
                    $job = Mage::getSingleton('job/comment')
                        ->load($jobId)
                        ->setStatus(2)
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d comment(s) were successfully approved', count($jobIds))
                );
            } catch (Exception $e) {
	...
	public function massUnapproveAction()
    {
        ...
        if(!is_array($jobIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d comment(s) were successfully unapproved', count($jobIds))
                );
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('job/comment')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('job/manage_comment_edit'));
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('job')->__('Unable to find comment to save'));
        $this->_redirect('*/*/');
	}

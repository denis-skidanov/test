...
class Den_Job_Helper_Comment extends Mage_Core_Helper_Abstract
{
     public function renderPage(Mage_Core_Controller_Front_Action $action, $identifier=null, $data)
    {
         $page = Mage::getSingleton('job/job');
		...
		$page_title = Mage::getSingleton('job/job')->load($identifier)->getTitle();
		...
		{
            Mage::helper('job')->addRss($action->getLayout()->getBlock('head'), Mage::getUrl(Mage::getStoreConfig('job/job/route')) . "rss");
		}
        */

 ...
 */
class Den_Job_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'job/job/enabled';
    const XML_PATH_TITLE = 'job/job/title';
    const XML_PATH_MENU_LEFT = 'job/job/menuLeft';
    const XML_PATH_MENU_RIGHT = 'job/job/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'job/job/footerEnabled';
    const XML_PATH_LAYOUT = 'job/job/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $jobRouter = Mage::getStoreConfig('job/job/route', $store->getId());
                    if ($jobRouter == $router) {
                        $droute = Mage::getStoreConfig('job/job/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('job/job/enabled') && $this->extensionEnabled('Den_Job');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('job/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

...
class Den_Staff_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        $front = $observer->getEvent()->getFront();

        $staff = new Den_Staff_Controller_Router();
        $front->addRouter('staff', $staff);
    }
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
		...
		}elseif (substr($identifier, 0, strlen('cat/')) == 'cat/')
		{
			...
			$cat = Mage::getSingleton('staff/cat');

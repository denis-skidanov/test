 ...
 */
class Den_Staff_Model_Observer {
    ...
    public function addBlogSection($observer) {
        ...
        if (!($sitemapObject instanceof Mage_Sitemap_Model_Sitemap))
            throw new Exception(Mage::helper('staff')->__('Error during generation sitemap'));

        $storeId = $sitemapObject->getStoreId();
        $date = Mage::getSingleton('core/date')->gmtDate('Y-m-d');
        $baseUrl = Mage::app()->getStore($storeId)->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_LINK);
        /**
         * Generate staff pages sitemap
         */
        $changefreq = (string) Mage::getStoreConfig('sitemap/staff/changefreq');
        $priority = (string) Mage::getStoreConfig('sitemap/staff/priority');
        $collection = Mage::getModel('staff/staff')->getCollection()->addStoreFilter($storeId);

...
class Den_Staff_Model_Mysql4_Staff extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('staff/staff', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on staff delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete staff/store
		$adapter->delete($this->getTable('staff/store'), 'post_id='.$object->getId());
		// 2. Delete staff/post_cat
		$adapter->delete($this->getTable('staff/post_cat'), 'post_id='.$object->getId());
		// 3. Delete staff/post_comment 
		$adapter->delete($this->getTable('staff/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('staff/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

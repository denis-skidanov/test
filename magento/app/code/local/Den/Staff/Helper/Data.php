 ...
 */
class Den_Staff_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'staff/staff/enabled';
    const XML_PATH_TITLE = 'staff/staff/title';
    const XML_PATH_MENU_LEFT = 'staff/staff/menuLeft';
    const XML_PATH_MENU_RIGHT = 'staff/staff/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'staff/staff/footerEnabled';
    const XML_PATH_LAYOUT = 'staff/staff/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $staffRouter = Mage::getStoreConfig('staff/staff/route', $store->getId());
                    if ($staffRouter == $router) {
                        $droute = Mage::getStoreConfig('staff/staff/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('staff/staff/enabled') && $this->extensionEnabled('Den_Staff');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('staff/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

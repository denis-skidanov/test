 ...
 */
class Den_Staff_Manage_StaffController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('admin/staff/posts');
    }

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('staff/posts');
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('staff/staff')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('staff/manage_staff_edit'))
                    ->_addLeft($this->getLayout()->createBlock('staff/manage_staff_edit_tabs'));
    ...
    public function duplicateAction() {
        ...
        while (Mage::getModel('staff/post')->loadByIdentifier($newIdentifier)->getData())
            $newIdentifier = $oldIdentifier . ++$i;
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('staff/post');
            if (isset($data['tags'])) {
                ...
                foreach ($tags as $key => $tag) {
                    $tags[$key] = Mage::helper('staff')->convertSlashes($tag, 'forward');
                }
            ...
            try {
                ...
                foreach ($affectedTags as $tag) {
                    foreach ($stores as $store) {
                        if (trim($tag)) {
                            Mage::getModel('staff/tag')->loadByName($tag, $store)->refreshCount();
                        }
                    }
                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('staff')->__('Post was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('staff')->__('Unable to find post to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('staff/staff');
                $model->load($this->getRequest()->getParam('id'));
                ...
                foreach ($_tags as $tag)
                    foreach ($_stores as $store)
                        if (trim($tag)) {
                            Mage::getModel('staff/tag')->loadByName($tag, $store->getId())->refreshCount();
                        }
        ...
        if (!is_array($staffIds)) {
            ...
            try {
                foreach ($staffIds as $staffId) {
                    $staff = Mage::getModel('staff/staff')->load($staffId);
                    $staff->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__(
                                'Total of %d record(s) were successfully deleted', count($staffIds)
                        )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction() {
        ...
        if (!is_array($staffIds)) {
            ...
            try {
                ...
                foreach ($staffIds as $staffId) {
                    $staff = Mage::getModel('staff/staff')
                            ->load($staffId)
    ...
    protected function displayTitle($data = null, $root = 'Staff') {
        ...
        if (!Mage::helper('staff')->magentoLess14()) {
            if ($data) {
                ...
                $this->_title($this->__('Staff'))->_title($root);
            }
        }
        return $this;
    }

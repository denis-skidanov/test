...
class Den_Staff_Manage_CatController extends Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/staff/cat');
    }

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('staff/cat')
        ...
        if(!is_array($staffIds)) {
            ...
            try {
                foreach ($staffIds as $staffId) {
                    $staff = Mage::getModel('staff/cat')->load($staffId);
                    $staff->delete();
                }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('staff/cat')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('staff/manage_cat_edit'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('staff')->__('Category does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('staff')->__('Unable to find category to save'));
        $this->_redirect('*/*/');
	}

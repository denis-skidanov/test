 ...
 */
class Den_Gloss_Manage_GlossController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('admin/gloss/posts');
    }

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('gloss/posts');
    ...
    public function editAction() {
        ...
        $model = Mage::getModel('gloss/gloss')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('gloss/manage_gloss_edit'))
                    ->_addLeft($this->getLayout()->createBlock('gloss/manage_gloss_edit_tabs'));
    ...
    public function duplicateAction() {
        ...
        while (Mage::getModel('gloss/post')->loadByIdentifier($newIdentifier)->getData())
            $newIdentifier = $oldIdentifier . ++$i;
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('gloss/post');
            if (isset($data['tags'])) {
                ...
                foreach ($tags as $key => $tag) {
                    $tags[$key] = Mage::helper('gloss')->convertSlashes($tag, 'forward');
                }
            ...
            try {
                ...
                foreach ($affectedTags as $tag) {
                    foreach ($stores as $store) {
                        if (trim($tag)) {
                            Mage::getModel('gloss/tag')->loadByName($tag, $store)->refreshCount();
                        }
                    }
                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('gloss')->__('Post was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('gloss')->__('Unable to find post to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function deleteAction() {
        if ($this->getRequest()->getParam('id') > 0) {
            try {
                $model = Mage::getModel('gloss/gloss');
                $model->load($this->getRequest()->getParam('id'));
                ...
                foreach ($_tags as $tag)
                    foreach ($_stores as $store)
                        if (trim($tag)) {
                            Mage::getModel('gloss/tag')->loadByName($tag, $store->getId())->refreshCount();
                        }
    ...
    public function massDeleteAction() {
        $glossIds = $this->getRequest()->getParam('gloss');
        if (!is_array($glossIds)) {
            ...
            try {
                foreach ($glossIds as $glossId) {
                    $gloss = Mage::getModel('gloss/gloss')->load($glossId);
                    $gloss->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__(
                                'Total of %d record(s) were successfully deleted', count($glossIds)
                        )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massStatusAction() {
        ...
        if (!is_array($glossIds)) {
            ...
            try {
                ...
                foreach ($glossIds as $glossId) {
                    $gloss = Mage::getModel('gloss/gloss')
                            ->load($glossId)
                ...
                $this->_getSession()->addSuccess(
                        $this->__('Total of %d record(s) were successfully updated', count($glossIds))
                );
            } catch (Exception $e) {
    ...
    protected function displayTitle($data = null, $root = 'Gloss') {
        ...
        if (!Mage::helper('gloss')->magentoLess14()) {
            if ($data) {
                ...
                $this->_title($this->__('Gloss'))->_title($root);
            }
        }
        return $this;
    }

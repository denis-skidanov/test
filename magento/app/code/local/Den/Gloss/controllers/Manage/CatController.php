...
class Den_Gloss_Manage_CatController extends Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/gloss/cat');
    }

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('gloss/cat')
        ...
        if(!is_array($glossIds)) {
            ...
            try {
                foreach ($glossIds as $glossId) {
                    $gloss = Mage::getModel('gloss/cat')->load($glossId);
                    $gloss->delete();
                }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('gloss/cat')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('gloss/manage_cat_edit'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('gloss')->__('Category does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('gloss')->__('Unable to find category to save'));
        $this->_redirect('*/*/');
	}

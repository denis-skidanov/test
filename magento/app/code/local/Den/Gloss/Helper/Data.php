 ...
 */
class Den_Gloss_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'gloss/gloss/enabled';
    const XML_PATH_TITLE = 'gloss/gloss/title';
    const XML_PATH_MENU_LEFT = 'gloss/gloss/menuLeft';
    const XML_PATH_MENU_RIGHT = 'gloss/gloss/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'gloss/gloss/footerEnabled';
    const XML_PATH_LAYOUT = 'gloss/gloss/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $glossRouter = Mage::getStoreConfig('gloss/gloss/route', $store->getId());
                    if ($glossRouter == $router) {
                        $droute = Mage::getStoreConfig('gloss/gloss/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('gloss/gloss/enabled') && $this->extensionEnabled('Den_Gloss');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('gloss/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

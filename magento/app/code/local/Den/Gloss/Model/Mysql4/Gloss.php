...
class Den_Gloss_Model_Mysql4_Gloss extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('gloss/gloss', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on gloss delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete gloss/store
		$adapter->delete($this->getTable('gloss/store'), 'post_id='.$object->getId());
		// 2. Delete gloss/post_cat
		$adapter->delete($this->getTable('gloss/post_cat'), 'post_id='.$object->getId());
		// 3. Delete gloss/post_comment 
		$adapter->delete($this->getTable('gloss/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('gloss/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

...
class Den_Reviews_Model_Mysql4_Reviews extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('reviews/reviews', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on reviews delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete reviews/store
		$adapter->delete($this->getTable('reviews/store'), 'post_id='.$object->getId());
		// 2. Delete reviews/post_cat
		$adapter->delete($this->getTable('reviews/post_cat'), 'post_id='.$object->getId());
		// 3. Delete reviews/post_comment 
		$adapter->delete($this->getTable('reviews/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('reviews/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

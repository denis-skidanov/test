 ...
 */
class Den_Reviews_Model_Api extends Mage_Core_Model_Abstract {
    ...
    public function getPostCategories($id) {
        ...
        return Mage::getModel('reviews/cat')
                        ->getCollection()
    ...
    public function getPosts($status = array(),$store = array()) {
        ...
        $collection = Mage::getModel('reviews/post')->getCollection();
    ...
    public function getPostShortContent($post,$storeId = 0) { 
        ...
        if (Mage::getStoreConfig(Den_Reviews_Helper_Config::XML_BLOG_USESHORTCONTENT,$storeId) && trim($post->getShortContent())) {             
            
                $content = trim($post->getShortContent());               
                 
            } elseif ((int) Mage::getStoreConfig(Den_Reviews_Helper_Config::XML_BLOG_READMORE, $storeId)) {
 
                $strManager = new Den_Reviews_Helper_Substring(array('input' => Mage::helper('reviews')->filterWYS($post->getPostContent())));

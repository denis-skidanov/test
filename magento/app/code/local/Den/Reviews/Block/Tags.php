...
class Den_Reviews_Block_Tags extends Mage_Core_Block_Template {


    public function _construct(){
        parent::_construct();
        return $this->setTemplate('aw_reviews/tags.phtml');
    }
    ...
    public function getCollection(){
        ...
        $coll = Mage::getModel('reviews/tag')->getCollection(); 
        $coll->getSelect()
                ->where('tag_count > 0')
                ->columns(array('tag_final_count'=>'SUM(tag_count)'))
                ->joinLeft(array("stores"=>$coll->getTable('reviews/store')),'main_table.store_id = stores.store_id',array())
                ->joinLeft(array("reviewss"=>$coll->getTable('reviews/reviews')),"stores.post_id = reviewss.post_id",array())
                ->where('reviewss.status = 1')
                ->where('FIND_IN_SET(main_table.tag, reviewss.tags)')
                ->where('main_table.store_id = ? OR main_table.store_id = 0', Mage::app()->getStore()->getId())
                ->order(array('tag_final_count DESC',
                           'tag'))
                           
                ->limit(Mage::getStoreConfig(Den_Reviews_Helper_Config::XML_TAGCLOUD_SIZE))
                ->group('tag');

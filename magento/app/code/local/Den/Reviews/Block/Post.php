 ...
 */
class Den_Reviews_Block_Post extends Mage_Core_Block_Template {
    ...
    public function getPost() {
        if (!$this->hasData('post')) {
            if ($this->getPostId()) {
                $post = Mage::getModel('reviews/post')
                                ->setStoreId(Mage::app()->getStore()->getId())
                                ->load($this->getPostId(), 'post_id');
            } else {
                $post = Mage::getSingleton('reviews/post');
            }
            
            /* Escape special chars */
            Den_Reviews_Helper_Data::escapeSpecialChars($post);
            /******************************/
            
            
            
            $cat = Mage::getSingleton('reviews/cat')->load($this->getRequest()->getParam('cat'), "identifier");
            ...
            $post->setCreatedTime($this->formatTime($post->getCreatedTime(), Mage::getStoreConfig('reviews/reviews/dateformat'), true));
            $post->setUpdateTime($this->formatTime($post->getUpdateTime(), Mage::getStoreConfig('reviews/reviews/dateformat'), true));
    ...
    public function getBookmarkHtml($post) {
        if (Mage::getStoreConfig('reviews/reviews/bookmarkspost')) {
    ...
    public function getComment() {
        ...
        $_perPage = Mage::helper('reviews/config')->getCommentsPerPage();
    ...
    public function getCommentTotalString($comments) {
        ...
        if ($comment_count == 1) {
            $comment_string = $comment_count . " " . Mage::helper('reviews')->__('Comment');
        } else {
            $comment_string = $comment_count . " " . Mage::helper('reviews')->__('Comments');
        }
    ...
    public function getLoginRequired() {
        return Mage::getStoreConfig('reviews/comments/login');
    }
    ...
    protected function _prepareLayout() {
        ...
        if (Mage::getStoreConfig('reviews/reviews/reviewscrumbs') && ($breadcrumbs = $this->getLayout()->getBlock('breadcrumbs'))) {
            ...
            if ($cat->getTitle() != "") {
                $breadcrumbs->addCrumb('cat', array('label' => $cat->getTitle(), 'title' => Mage::helper('reviews')->__('Return to ' . $cat->getTitle()), 'link' => Mage::getUrl($route . '/cat/' . $cat->getIdentifier())));
            }
            $breadcrumbs->addCrumb('reviews_page', array('label' => htmlspecialchars_decode($post->getTitle())));
        }
    ...
    public function getCommentText() {
        $reviewsPostModelFromSession = Mage::getSingleton('customer/session')->getReviewsPostModel();
        if ($reviewsPostModelFromSession)
            return $reviewsPostModelFromSession->getComment();
    ...
    public function getCommentEmail() {
        $reviewsPostModelFromSession = Mage::getSingleton('customer/session')->getReviewsPostModel();
        if ($reviewsPostModelFromSession)
            return $reviewsPostModelFromSession->getEmail();
    ...
    public function getCommentName() {
        $reviewsPostModelFromSession = Mage::getSingleton('customer/session')->getReviewsPostModel();
        if ($reviewsPostModelFromSession)
            return $reviewsPostModelFromSession->getUser();

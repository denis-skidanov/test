...
class Den_Reviews_Block_Rss extends Mage_Rss_Block_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		if (Mage::getStoreConfig('reviews/rss/image') != "")
		{
			$data['image'] = $this->getSkinUrl(Mage::getStoreConfig('reviews/rss/image'));
		}
		...
		if ($cat_id = Mage::getSingleton('reviews/cat')->load($identifier)->getcatId()){
			Mage::getSingleton('reviews/status')->addCatFilterToCollection($collection, $cat_id);
		}
		
		
		Mage::getSingleton('reviews/status')->addEnabledFilterToCollection($collection);
		
		$collection->setPageSize((int)Mage::getStoreConfig('reviews/rss/posts'));

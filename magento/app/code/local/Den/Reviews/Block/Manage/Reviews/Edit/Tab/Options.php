 ...
 */
class Den_Reviews_Block_Manage_Reviews_Edit_Tab_Options extends Mage_Adminhtml_Block_Widget_Form {

    protected function _prepareForm() {
        $form = new Varien_Data_Form();
        $this->setForm($form);
        $fieldset = $form->addFieldset('reviews_form', array('legend' => Mage::helper('reviews')->__('Meta Data')));

        $fieldset->addField('meta_keywords', 'editor', array(
            'name' => 'meta_keywords',
            'label' => Mage::helper('reviews')->__('Keywords'),
            'title' => Mage::helper('reviews')->__('Meta Keywords'),
            'style' => 'width: 520px;',
        ));

        $fieldset->addField('meta_description', 'editor', array(
            'name' => 'meta_description',
            'label' => Mage::helper('reviews')->__('Description'),
            'title' => Mage::helper('reviews')->__('Meta Description'),
            'style' => 'width: 520px;',
        ));

        $fieldset = $form->addFieldset('reviews_options', array('legend' => Mage::helper('reviews')->__('Advanced Post Options')));

        $fieldset->addField('user', 'text', array(
            'label' => Mage::helper('reviews')->__('Poster'),
            'name' => 'user',
            'style' => 'width: 520px;',
        ...
        } elseif ($data = Mage::registry('reviews_data')) {

 ...
 */
class Den_Reviews_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'reviews/reviews/enabled';
    const XML_PATH_TITLE = 'reviews/reviews/title';
    const XML_PATH_MENU_LEFT = 'reviews/reviews/menuLeft';
    const XML_PATH_MENU_RIGHT = 'reviews/reviews/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'reviews/reviews/footerEnabled';
    const XML_PATH_LAYOUT = 'reviews/reviews/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $reviewsRouter = Mage::getStoreConfig('reviews/reviews/route', $store->getId());
                    if ($reviewsRouter == $router) {
                        $droute = Mage::getStoreConfig('reviews/reviews/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('reviews/reviews/enabled') && $this->extensionEnabled('Den_Reviews');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('reviews/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

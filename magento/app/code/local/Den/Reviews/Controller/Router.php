...
class Den_Reviews_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        $front = $observer->getEvent()->getFront();

        $reviews = new Den_Reviews_Controller_Router();
        $front->addRouter('reviews', $reviews);
    }
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
		...
		}elseif (substr($identifier, 0, strlen('cat/')) == 'cat/')
		{
			...
			$cat = Mage::getSingleton('reviews/cat');

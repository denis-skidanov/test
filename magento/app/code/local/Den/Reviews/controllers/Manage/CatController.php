...
class Den_Reviews_Manage_CatController extends Mage_Adminhtml_Controller_Action
{
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/reviews/cat');
    }

	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('reviews/cat')
	...
	public function deleteAction() {
		if( $this->getRequest()->getParam('id') > 0 ) {
			try {
				$model = Mage::getModel('reviews/cat');
        ...
        if(!is_array($reviewsIds)) {
            ...
            try {
                foreach ($reviewsIds as $reviewsId) {
                    $reviews = Mage::getModel('reviews/cat')->load($reviewsId);
                    $reviews->delete();
                }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('reviews/cat')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('reviews/cat');
            $this->displayTitle('Edit category');

			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Reviews Manager'), Mage::helper('adminhtml')->__('ReviewsManager'));
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Category Manager'), Mage::helper('adminhtml')->__('Category Manager'));

			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

			$this->_addContent($this->getLayout()->createBlock('reviews/manage_cat_edit'));

			$this->renderLayout();
		} else {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('reviews')->__('Category does not exist'));
			$this->_redirect('*/*/');
		}
	}
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('reviews')->__('Unable to find category to save'));
        $this->_redirect('*/*/');
	}

 ...
 */
class Den_Reviews_Manage_ReviewsController extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _isAllowed() {
        return Mage::getSingleton('admin/session')->isAllowed('admin/reviews/posts');
    }

    protected function _initAction() {
        $this->loadLayout()
                ->_setActiveMenu('reviews/posts');
    ...
    public function editAction() {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('reviews/manage_reviews_edit'))
    ...
    public function duplicateAction() {
        ...
        while (Mage::getModel('reviews/post')->loadByIdentifier($newIdentifier)->getData())
            $newIdentifier = $oldIdentifier . ++$i;
    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('reviews/post');
            if (isset($data['tags'])) {
                ...
                foreach ($tags as $key => $tag) {
                    $tags[$key] = Mage::helper('reviews')->convertSlashes($tag, 'forward');
                }
            ...
            try {
                ...
                foreach ($affectedTags as $tag) {
                    foreach ($stores as $store) {
                        if (trim($tag)) {
                            Mage::getModel('reviews/tag')->loadByName($tag, $store)->refreshCount();
                        }
                    }
                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('reviews')->__('Post was successfully saved'));
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('reviews')->__('Unable to find post to save'));
        $this->_redirect('*/*/');
    }
            ...
            try {
                ...
                foreach ($_tags as $tag)
                    foreach ($_stores as $store)
                        if (trim($tag)) {
                            Mage::getModel('reviews/tag')->loadByName($tag, $store->getId())->refreshCount();
                        }
        ...
        if (!is_array($reviewsIds)) {
            ...
            try {
                foreach ($reviewsIds as $reviewsId) {
                    $reviews = Mage::getModel('reviews/reviews')->load($reviewsId);
                    $reviews->delete();
                }

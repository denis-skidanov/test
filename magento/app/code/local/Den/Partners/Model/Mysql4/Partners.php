...
class Den_Partners_Model_Mysql4_Partners extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct(){    
        $this->_init('partners/partners', 'post_id');
    }
    ...
    protected function _beforeDelete(Mage_Core_Model_Abstract $object){
		...
		// Cleanup stats on partners delete
		$adapter = $this->_getReadAdapter();
		// 1. Delete partners/store
		$adapter->delete($this->getTable('partners/store'), 'post_id='.$object->getId());
		// 2. Delete partners/post_cat
		$adapter->delete($this->getTable('partners/post_cat'), 'post_id='.$object->getId());
		// 3. Delete partners/post_comment 
		$adapter->delete($this->getTable('partners/comment'), 'post_id='.$object->getId());
		// Update tags
		
		try{
			$tags = explode(",",$object->getTags());
			if(count($tags)){
				Mage::getModel('partners/tag')->loadByName($tag, null)->refreshCount();
			}
		}catch(Exception $e){}	
	}
}

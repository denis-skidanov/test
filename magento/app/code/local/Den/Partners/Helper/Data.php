 ...
 */
class Den_Partners_Helper_Data extends Mage_Core_Helper_Abstract {
    const XML_PATH_ENABLED = 'partners/partners/enabled';
    const XML_PATH_TITLE = 'partners/partners/title';
    const XML_PATH_MENU_LEFT = 'partners/partners/menuLeft';
    const XML_PATH_MENU_RIGHT = 'partners/partners/menuRoght';
    const XML_PATH_FOOTER_ENABLED = 'partners/partners/footerEnabled';
    const XML_PATH_LAYOUT = 'partners/partners/layout';
        ...
        if (isset($params['___from_store'])) {
            ...
            foreach ($stores as $store) {
                if ($store->getCode() == $storeName) {
                    $partnersRouter = Mage::getStoreConfig('partners/partners/route', $store->getId());
                    if ($partnersRouter == $router) {
                        $droute = Mage::getStoreConfig('partners/partners/route', Mage::app()->getStore()->getId());
    ...
    public function getEnabled() {
        return Mage::getStoreConfig('partners/partners/enabled') && $this->extensionEnabled('Den_Partners');
    }
    ...
    public function getRssEnabled() {
        return (Mage::getStoreConfigFlag('partners/rss/enable') && Mage::getStoreConfigFlag('rss/config/active'));
    }

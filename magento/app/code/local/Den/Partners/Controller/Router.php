...
class Den_Partners_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        $front = $observer->getEvent()->getFront();

        $partners = new Den_Partners_Controller_Router();
        $front->addRouter('partners', $partners);
    }
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
		...
		if($identifier == '')
		{
			$request->setModuleName('partners')
		...
		}elseif (substr($identifier, 0, strlen('cat/')) == 'cat/')
		{
			...
			$cat = Mage::getSingleton('partners/cat');

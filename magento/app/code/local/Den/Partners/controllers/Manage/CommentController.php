...
class Den_Partners_Manage_CommentController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _isAllowed(){
        return Mage::getSingleton('admin/session')->isAllowed('admin/partners/comment');
    }
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('partners/comment')
    ...
    public function massDeleteAction() {
        ...
        $partnersIds = $this->getRequest()->getParam('comment');
        if(!is_array($partnersIds)) {
            ...
            try {
                foreach ($partnersIds as $partnersId) {
                    $partners = Mage::getModel('partners/comment')->load($partnersId);
                    $partners->delete();
                }
    ...
    public function massApproveAction()
    {
        ...
        if(!is_array($partnersIds)) {
            ...
            try {
                foreach ($partnersIds as $partnersId) {
                    $partners = Mage::getSingleton('partners/comment')
                        ->load($partnersId)
                        ->setStatus(2)
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d comment(s) were successfully approved', count($partnersIds))
                );
            } catch (Exception $e) {
	...
	public function massUnapproveAction()
    {
        ...
        if(!is_array($partnersIds)) {
            ...
            try {
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d comment(s) were successfully unapproved', count($partnersIds))
                );
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage());
            }
	...
	public function editAction() {
		...
		$model  = Mage::getModel('partners/comment')->load($id);
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('partners/manage_comment_edit'));
	...
	public function saveAction() {
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('partners')->__('Unable to find comment to save'));
        $this->_redirect('*/*/');
	}

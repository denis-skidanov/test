...
class Den_Partners_Block_Rss extends Mage_Rss_Block_Abstract
{
    ...
    protected function _toHtml()
    {
		...
		if (Mage::getStoreConfig('partners/rss/image') != "")
		{
			$data['image'] = $this->getSkinUrl(Mage::getStoreConfig('partners/rss/image'));
		}
		...
		if ($cat_id = Mage::getSingleton('partners/cat')->load($identifier)->getcatId()){
			Mage::getSingleton('partners/status')->addCatFilterToCollection($collection, $cat_id);
		}
		
		
		Mage::getSingleton('partners/status')->addEnabledFilterToCollection($collection);
		
		$collection->setPageSize((int)Mage::getStoreConfig('partners/rss/posts'));

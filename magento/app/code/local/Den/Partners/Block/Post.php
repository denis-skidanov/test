 ...
 */
class Den_Partners_Block_Post extends Mage_Core_Block_Template {
    ...
    public function getPost() {
        if (!$this->hasData('post')) {
            if ($this->getPostId()) {
                $post = Mage::getModel('partners/post')
                                ->setStoreId(Mage::app()->getStore()->getId())
                                ->load($this->getPostId(), 'post_id');
            } else {
                $post = Mage::getSingleton('partners/post');
            }
            
            /* Escape special chars */
            Den_Partners_Helper_Data::escapeSpecialChars($post);
            /******************************/
            
            
            
            $cat = Mage::getSingleton('partners/cat')->load($this->getRequest()->getParam('cat'), "identifier");
            ...
            $post->setCreatedTime($this->formatTime($post->getCreatedTime(), Mage::getStoreConfig('partners/partners/dateformat'), true));
            $post->setUpdateTime($this->formatTime($post->getUpdateTime(), Mage::getStoreConfig('partners/partners/dateformat'), true));
    ...
    public function getBookmarkHtml($post) {
        if (Mage::getStoreConfig('partners/partners/bookmarkspost')) {
    ...
    public function getComment() {
        ...
        $_perPage = Mage::helper('partners/config')->getCommentsPerPage();
    ...
    public function getCommentTotalString($comments) {
        ...
        if ($comment_count == 1) {
            $comment_string = $comment_count . " " . Mage::helper('partners')->__('Comment');
        } else {
            $comment_string = $comment_count . " " . Mage::helper('partners')->__('Comments');
        }
    ...
    public function getLoginRequired() {
        return Mage::getStoreConfig('partners/comments/login');
    }
    ...
    protected function _prepareLayout() {
        ...
        if (Mage::getStoreConfig('partners/partners/partnerscrumbs') && ($breadcrumbs = $this->getLayout()->getBlock('breadcrumbs'))) {
            ...
            if ($cat->getTitle() != "") {
                $breadcrumbs->addCrumb('cat', array('label' => $cat->getTitle(), 'title' => Mage::helper('partners')->__('Return to ' . $cat->getTitle()), 'link' => Mage::getUrl($route . '/cat/' . $cat->getIdentifier())));
            }
            $breadcrumbs->addCrumb('partners_page', array('label' => htmlspecialchars_decode($post->getTitle())));
        }
    ...
    public function getCommentText() {
        $partnersPostModelFromSession = Mage::getSingleton('customer/session')->getPartnersPostModel();
        if ($partnersPostModelFromSession)
            return $partnersPostModelFromSession->getComment();
    ...
    public function getCommentEmail() {
        $partnersPostModelFromSession = Mage::getSingleton('customer/session')->getPartnersPostModel();
        if ($partnersPostModelFromSession)
            return $partnersPostModelFromSession->getEmail();
    ...
    public function getCommentName() {
        $partnersPostModelFromSession = Mage::getSingleton('customer/session')->getPartnersPostModel();
        if ($partnersPostModelFromSession)
            return $partnersPostModelFromSession->getUser();
